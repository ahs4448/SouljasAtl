...
let slideIndex = [1, 1, 1]; // Current slide index for each card
let slideId = [0, 1, 2]; // The card IDs
let slideInterval; // Variable to hold the interval ID

function showSlides(n, no) {
    let i;
    let slides = document.getElementsByClassName("mySlides");
    let totalSlides = slides.length / slideId.length;

    for (i = 0; i < totalSlides; i++) {
        slides[i + (no * totalSlides)].style.display = "none";  
    }
    
    slideIndex[no] += n;

    if (slideIndex[no] > totalSlides) {slideIndex[no] = 1}    
    if (slideIndex[no] < 1) {slideIndex[no] = totalSlides}
    
    slides[(slideIndex[no] - 1) + (no * totalSlides)].style.display = "block";  
}

function plusSlides(n, no) {
    clearInterval(slideInterval); // Clear the interval on manual navigation
    showSlides(n, no);
    startSlideInterval(no); // Restart the interval
}

function startSlideInterval(no) {
...
for (let i = 0; i < slideId.length; i++) {
    showSlides(0, i);
    startSlideInterval(i); // Start the interval for each card
}
